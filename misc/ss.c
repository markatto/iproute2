...
static void sk_opts_show(struct rtattr *attr)
{
	...
	if (!attr || RTA_PAYLOAD(attr) < sizeof(bf))
		return;
	memcpy(&bf, RTA_DATA(attr), sizeof(bf));
	val = bf.value & bf.selector;

	if (!oneline)
		out("\n\tsk-opts: (");
	else
		out(" sk-opts: (");
	if (val & SK_DIAG_OPT_REUSEADDR)
		out(" reuseaddr");
	if (val & SK_DIAG_OPT_REUSEPORT)
		out(" reuseport");
	if (val & SK_DIAG_OPT_KEEPALIVE)
		out(" keepalive");
	if (val & SK_DIAG_OPT_BROADCAST)
		out(" broadcast");
	if (val & SK_DIAG_OPT_OOBINLINE)
		out(" oobinline");
	if (val & SK_DIAG_OPT_DONTROUTE)
		out(" dontroute");
	if (val & SK_DIAG_OPT_LINGER)
		out(" linger");
	if (val & SK_DIAG_OPT_TIMESTAMP)
		out(" timestamp");
	if (val & SK_DIAG_OPT_TIMESTAMPNS)
		out(" timestampns");
	if (val & SK_DIAG_OPT_DEBUG)
		out(" debug");
	if (val & SK_DIAG_OPT_ZEROCOPY)
		out(" zerocopy");
	if (val & SK_DIAG_OPT_TXTIME)
		out(" txtime");
	if (val & SK_DIAG_OPT_RXQ_OVFL)
		out(" rxq_ovfl");
	if (val & SK_DIAG_OPT_SELECT_ERR_QUEUE)
		out(" select_err_queue");
	if (val & SK_DIAG_OPT_NOFCS)
		out(" nofcs");
	if (val & SK_DIAG_OPT_RCVMARK)
		out(" rcvmark");
	out(")");
}
...
static int unix_show_netlink(struct filter *f)
{
	...
	if (show_sock_opt)
		req.r.udiag_show |= UDIAG_SHOW_SK_OPTS;
...
static int packet_show_netlink(struct filter *f)
{
	...
	if (show_sock_opt)
		req.r.pdiag_show |= PACKET_SHOW_SK_OPTS;
...
static int xdp_show(struct filter *f)
{
	...
	if (show_sock_opt)
		req.r.xdiag_show |= XDP_SHOW_SK_OPTS;
...
static int netlink_show_netlink(struct filter *f)
{
	...
	if (show_sock_opt)
		req.r.ndiag_show |= NDIAG_SHOW_SK_OPTS;
